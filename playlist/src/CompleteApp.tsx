    ...
    await fetch(artisturl, { headers })
      ...
      .then(async (artistres) => {
        ...
        setTopArtistInfo({
          ...topArtistInfo,
          artist: {
            name: artistres.items[0].name,
            pic: artistres.items[0].images[0],
            external_urls: artistres.items[0].external_urls
          }
        })
        ...
        <Grid justifyContent="center" container direction="row" spacing={1}>
          {connectedRoom! && (
            <Grid justifyContent="center" container item direction="row" ml={2}>
              <Typography mt={2} pl={1} pr={2}>
                Current room: {connectedRoom}
              </Typography>
            </Grid>
          )}
          {recentPlaylist! && (
            <Grid justifyContent="center" container item direction="row" ml={2}>
              <Typography mt={2} pl={1} pr={2}>
                recent playlist: {connectedRoom}
              </Typography>
              <Link mt={2} pl={1} pr={2} style={{ textAlign: 'center' }} href={recentPlaylist}>
                here!
              </Link>
            </Grid>
          )}
          ...
          <Grid justifyContent="center" container item direction="row">
            <Typography mt={2} pl={1} pr={2}>
              Your playlist will be based on:
            </Typography>
          </Grid>
          {topArtistInfo.artist && (
            <Grid justifyContent="center" container item direction="row" ml={2}>
              <Link
                ...
                href={topArtistInfo.external_urls ? `${topArtistInfo.external_urls.spotify}` : ''}>
                {topArtistInfo.artist.name}
              </Link>
            ...
            <>
              <Grid justifyContent="center" container item direction="row">
                <Link
                  ...
                  pr={2}
                  style={{ textAlign: 'center' }}
                  href={topSongsInfo[0].external_urls.spotify ? `${topSongsInfo[0].external_urls.spotify}` : ''}>
                  {topSongsInfo[0].name}
                </Link>
                <Avatar
                  sx={{ width: 60, height: 60 }}
                  alt="favorite artist"
                  src={topSongsInfo[0].album.images[0].url}
                />
              </Grid>
              <Grid justifyContent="center" container item direction="row">
                <Link
                  ...
                  pr={2}
                  style={{ textAlign: 'center' }}
                  href={topSongsInfo[0].external_urls.spotify ? `${topSongsInfo[1].external_urls.spotify}` : ''}>
                  {topSongsInfo[1].name}
                </Link>
                <Avatar
                  sx={{ width: 60, height: 60 }}
                  alt="favorite artist"
                  src={topSongsInfo[1].album.images[1].url}
                />
              </Grid>
            </>
          )}
  ...
  return (
    <Grid
      ...
      <Grid container item justifyContent="center">
        ...
        <Button size="small" onClick={() => JoinRoom(roomCode)}>
      ...
      <Grid container item justifyContent="center">
        <Button size="small" onClick={() => CreateGroupPlaylist()}>
